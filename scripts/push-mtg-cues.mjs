...
try {
  await remote.execute("SELECT artist, frame, flavor_name, art_hash FROM mtg_cards LIMIT 1");
} catch (err) {
...
for (let i = 0; i < rows.length; i += BATCH) {
  ...
  await remote.batch(
    chunk.map((r) => ({
      sql: `UPDATE mtg_cards SET artist = ?, frame = ?, border_color = ?, frame_effects = ?, promo_types = ?, full_art = ?, textless = ?, flavor_name = ?, art_hash = ? WHERE id = ?`,
      args: [r.artist, r.frame, r.border_color, r.frame_effects, r.promo_types, r.full_art, r.textless, r.flavor_name, r.art_hash ?? "", r.id],
    })),
    "write",
  );
  done += chunk.length;
