...
- [x] FOLDER MANAGEMENT (09-08, Chris: "management options for the folders"): pencil button at the end of the category chips → components/CategoryManager.tsx (rename inline — typing an existing name merges; delete → cards go Uncategorized, confirm first). POST /api/cards/categories {action: rename|delete, from, to} → cards.ts renameCategory/clearCategory (one UPDATE). CATEGORY_MAX moved to lib/categories.ts. test:cards pins rename/merge/delete + ownership.
