...
- [ ] Stripe polish: ~~branding logo/color~~ DONE 09-02 (per mega-day; Chris confirmed 09-02 eve — business name "CardFlip" also already fixed, stale note removed). Remaining: verify Settings→Emails "Successful payments" toggle; OPTIONAL sk_live rotation (chat-exposed 09-01; roll in dashboard + rerun scripts/flip-stripe-live.mjs). Turso tokens also chat-exposed — Chris accepted that risk 08-31.
...
- [ ] Graded-chart transition polish round 2 (Chris 09-02 eve: "better, not perfect, later") — candidates: prefetch common grades on grade-panel open, skeleton shimmer during first fetch, tween the y-axis ticks with the curve
- [ ] Graded price history accumulation: record each graded-comps average as a price_series point (variant e.g. "psa10-asking") so tracked cards grow a real graded curve; chart switches from estimate to real points once enough exist
