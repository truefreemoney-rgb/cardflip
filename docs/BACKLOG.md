...
- [x] PHONE HEADER = TWO ROWS (09-07, Chris: "do the header too"): row 1 logo + Help robot + ✦ invite (icon-only, sr-only label) + eBay chip; row 2 the tab pill (py-2). Name and Sign out are sm+ only; phones sign out from Account → Support → Sign out (new row). ~100px tall at 375 (was ~300). sm+ and xl layouts unchanged.
- [x] INVENTORY TEXT ROW MAKEOVER (09-07, Chris: "the inventory page on mobile is an absolute mess"): one shape per row — checkbox · art · name/set/condition + state chip + dates (doubt as its own amber line) · price pinned top-right ($ restored — the live-prices commit had dropped it) · ONE action row (primary stretches on phones: Verify Match / Build Listing / Relist; quiet: End Auction, View on eBay, Delete). "Draft" pill and the "View on eBay" inline link are gone; sm+ puts the same pieces on one line. Awaiting yea/nay on the phone.
