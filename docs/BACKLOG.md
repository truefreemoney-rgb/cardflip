...
- [ ] PSA: prod retest PARKED 09-02 morning (Chris has no graded slab on hand; demo-login path is blocked by our own fix, so the test needs his real session — note: no physical slab strictly needed, any card marked Graded-by-PSA + cert 28400235 in the Verify field works, when he's up for it). NEW FINDING 09-02 am: PSA 429s "quota exceeded 100/day" even on a GARBAGE token (local probe, scripts/psa-quota-check.mjs) → a 429 proves nothing about OUR token/quota; reset is ~6am UTC (2am ET), not 6am ET. Still watching support@cardflip.io for collectors-apis reply (sent 09-01). Also: local .env.vercel.local TURSO creds stale ("group not found") — db counters only readable via prod.
