...
- [x] S — Graded cert lookup — PSA SHIPPED 09-01 (lib/server/psa.ts + /api/psa/cert, Verify field in CardEditor when Graded-by-PSA; PSA_API_TOKEN on Vercel prod+preview; 10/min/user + 80/day global vs the 100/day free tier; awaiting Chris real-slab test on prod). CGC: no public API — parked unless Chris finds one
