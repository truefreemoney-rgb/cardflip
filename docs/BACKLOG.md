...
- [ ] (dup of §0.0 A) Anthropic credits auto-reload — CHRIS 1-min errand: console.anthropic.com → Billing → enable auto-reload (ran dry once 09-01; optional)
- [ ] **MTG Art Series USD prices** — art cards are in the mirror now (scripts/sync-mtg-art.mjs, 2,650 across 40 sets, in prod 09-03) but Scryfall prices them in EUR only; the gold-signature variant is the "foil" finish. Needs a TCGplayer product map for MTG (tcgcsv groups "… Art Series") to price in USD. Until then the editor quotes the EUR reference.
- [ ] **MTG stress test** (Chris 09-03: "going to be a pain"; he will say when). Pre-flight first (~1h): extend scripts/replay-scans.mjs to MTG (searchMtgCardsLocal), synthetic pass over Scryfall art for: no-number vintage, DFC/adventure/split names, foil/etched/showcase printings, The List / Secret Lair codes. Mirror is healthy: 94,144 printings, 0 missing art, ~2% unpriced.
