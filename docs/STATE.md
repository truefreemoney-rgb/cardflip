...
Prod is v109, healthy; admin login admin/onyx VERIFIED on prod. **Deploys are now MINE (Chris 08-17: his non-technical assistant is finishing the project; never hand out shell commands) — after each commit: `flyctl deploy --app cardflip-superior`, then `flyctl releases` + one curl.** v116 LIVE (08-17, Chris: "go crazy and work on user interaction, make the site smooth and make sense"): shared `AppHeader`+`SessionProvider` (no blank flash, one auth fetch, `PageSkeleton`), `Toaster` bus, `PasswordField` on auth pages, `useFocusTrap` on modals, signup `?step=ebay`, "Search cards" naming, scanner 3-step strip. §6 QoL now fully done (08-17 polish: queue survives refresh via queuePersistence.ts, stale-search guard, chip pulse exemption, lazy card images). v114: **account settings page** `/app/account` (person icon at end of AppTabs; Chris: "work on user functionality like account settings") — see BACKLOG §3. Also v113: (a) peek-modal price chart + wishlist sparklines (`PriceSparkline.tsx`; `wishlist_items` gained `card_id`/`game`), (b) **wrong-match fix** — Chris's phone scan of a full-art Sprigatito (PAL 196/193) matched SVP 001 promo because vision returned no number and every printing tied → oldest-first. Now vision returns `artStyle` ("standard"|"full-art"), passed as `?art=` to `/api/search-card`, `enCards.ts` ART_PENALTY tiebreaks against `isSecretRareNumber` (full-art ⇔ numbered above set total; mirror has no rarity column). Verified on dev: `Sprigatito&art=full-art` → PAL 196/193 first. Step 1: Chris (or his assistant) **rescans that Sprigatito** on prod — if still wrong, vision isn't reading full-art/number on that photo; next lever = rarity column in the mirror (needs `sync:en` re-run) or ask him for the photo. Then he checks a landing card peek + wishlist sparkline on prod. Step 2: next BACKLOG.md items needing no Chris input — §6 open QoL (shared `<AppHeader>` + server session, scan-queue persistence in sessionStorage), §4 auth/db tests; §7 pinger needs a scheduled task (ask him). Chris 08-16 late: "ok calm it down" = fewer tool calls / less browser poking per task — verify with tsc+lint and one check, then report. Do NOT re-read the long notes below unless a task needs them.
