...
**UPDATE 09-26 ~11:45am ET (session 37, later): (1) DIP EMAIL DID NOT GO OUT — the 5:45am Pokémon cron was killed at Vercel's 300s limit inside sweepPriceHistory (marker last_sweep_at 09:45:46Z, no daily_last_result written), so eBay sync + wishlist alerts + result never ran. Fixed 00cd9c1: wishlist alerts run right after the TCGCSV refresh, sweepPriceHistory(now, now+200_000) deadline. PROOF PENDING: Chris presses Run now on /admin/system (or 09-27 5:45am) → email to truefreemoney+dip402@gmail.com → he says "dip email arrived" → delete the Dip Test account. SMTP (Fastmail 587) verified working from here. (2) MONEY FORMAT 62fc056: every shown $ goes through formatMoney (thousands separators, Chris rule "sitewide"); inputs/eBay payloads/CSV untouched; CI green. Prod env pulled to the scratchpad only (CRON_SECRET is [SENSITIVE]).**

**FIRST ACTION (saved 09-26 ~10:40am ET, session 37). CI GREEN AGAIN (72fc05b: test:settings read Pro=2000 scans, ladder is 750; it was red on every push since the 1am pricing change, ten runs, Chris saw the emails). Shipped this session, all on main: runner upgrade 7 (fec9691 /api/runner/photo hands board photos to the runner; upgrades 1/2/3/6 were already shipped 09-10 — rows closed); homepage demo frame in the Inventory tile (four big Pokémon cards Scanned/Priced/Listed/Sold with EXAMPLE tags, Chris yea after two rounds: list rows were too tiny); MAGIC IS ADMIN-ONLY BETA (Chris 09-26: "everything should be pokemon only") — the card wall ignored the switch, f92168a gates getShowcaseCards(magic); USD ONLY (a18ef9d: homepage variants no EUR row, price chart no EUR fallback); watchlist search results got a sort (best match / price / set) and Clear → "Close results" (f41944f). eBay Marketplace Insights: CLOSED TO NEW USERS per eBay docs (checked logged in) — nothing to apply to, board row says re-check ~3 months. NEXT: (a) Chris: "dip email arrived" / "no dip email" (truefreemoney+dip402@gmail.com from the 5:45am sweep) → delete the Dip Test account or read Vercel logs; (b) confirm the 1pm/7pm social posts fired on Vercel's clock (social_slot keys / Vercel logs); (c) tomorrow 7am first MOVERS video — look at it; (d) Pinterest = Chris's gated task (account first); (e) token refresh step for Facebook/Instagram/Threads before mid-Nov; (f) Anthropic downgrade decision: mention once ~Oct 6, not before. Chris pauses on runner work: only do runner rows he names.**

**FIRST ACTION (saved 09-26 ~9:30am ET, session 36). SOCIAL SCHEDULE MOVED TO VERCEL CRON + VIDEO NOW MATCHES TEXT. Morning: GitHub cron fired 2h late, 7am post went out at 8:55 by hand (video on X/FB/IG/Threads; Bluesky picture = "unconfirmed_email", Chris confirmed the email since; TikTok skipped, 1am test post counted). Chris: "these posts must go out 7am/1pm/7pm no matter what". Shipped: (1) cd4a651 vercel.json crons → /api/social/publish at 7/1/7 ET both DST hours (Pro plan, on the minute); render pings on GitHub from 4:30/5:30/6:15am; /api/cron/social-video at 6:40 dispatches render_only when nothing is registered — PROVEN: GITHUB_TOKEN on Vercel replaced with a classic PAT (repo+workflow), forced test dispatched run 36244717176. (2) d5c0a23 two bars per card (5.2s, 31.7s for five; Chris: too fast). (3) df43f9f (Sonnet agent): morning=MOVERS video (No.5→No.1 by % gain), midday=set picture, evening=dips; render freezes its card list into VideoSpec.cards and the publisher writes the caption from it (video ≠ text bug: set hash index shifted after the 5:45 price cron); same-day dedupe by kind (social_kind:<site>:<kind>) rotates movers→set→dips; I backfilled social_kind:*:set=2026-09-26 by hand so 1pm today is NOT Mysterious Treasures again. NEXT: (a) confirm 1pm/7pm went out on Vercel's clock (Vercel logs or the social_slot keys; there is no GitHub run for them any more); (b) tomorrow 7am: first MOVERS video — LOOK at it (cards = text? slower cut? Bluesky video=yes now?); (c) Chris's Anthropic billing ticket 215476113093380, downgrade to Max 5x before Oct 8; (d) Pinterest (Chris's gated task); (e) TikTok audit pending. Board fix below still true.**

**UPDATE 09-26 ~2:30am ET (session 36): BOARD WAS RESEEDING FROM docs/BOARD.md ON EVERY LOAD since ~09-10 (a 1516-char row failed the 1000-char cap → loadBoard fell through to the file seed each request). Symptoms Chris saw: done rows stuck in their live section, "Completed · 25" while the DB had 35, and any edit he made on the live board vanished on the next load. Fixed in 2df49b4 (ITEM_TEXT_MAX 4000, stored board never replaced because of validation, test:board validates the real file). VERIFY after deploy: GET /api/admin/board from the admin tab → 0 done rows outside Completed, Completed = 35, updatedAt stable across two GETs. Chris also filed an Anthropic billing ticket (Max 5x→20x upgrade $123.95, conversation 215476113093380); he will downgrade to 5x before Oct 8. Stray page.tsx max-w-4xl edit was NOT his → reverted. Brand PNGs in public/brand are his, leave untracked.**

**FIRST ACTION (saved 09-26 ~1:45am ET, session 35). TIKTOK SUBMITTED FOR REVIEW; POSTING PRIVATELY VIA SANDBOX. Done tonight: (1) referral bonus 500 -> 100 (PRICING.referral in src/lib/pricing.ts, 3217157). (2) TikTok adapter src/lib/server/sites/tiktok.ts (video only: publisher skips slots with no MP4, no picture fallback; SocialSite gained videoOnly/authorized()/connectPath), OAuth routes api/social/tiktok/connect + callback, tokens in settings social_token:tiktok (self-refreshing), per-site "post" link on /admin/social (= ?site=<id>&slot=morning&force=1), TikTok error code in messages. (3) Chris made account @cardflipio (PRIVATE for now, TikTok rule for unaudited apps), org CardFlip, dev app CardFlip (Login Kit + Content Posting API Direct Post, scopes user.info.basic/video.upload/video.publish, redirect https://cardflip.io/api/social/tiktok/callback), site verification files in public/, a SANDBOX "CardFlip Sandbox" with cardflipio as target user. Vercel prod TIKTOK_CLIENT_KEY/SECRET = the SANDBOX keys (production keys were pasted in chat session 35; Chris re-pastes them when the audit passes). Connected + first private post landed (video init 403 until the account was private). Demo recorded (Downloads/cardflip-tiktok-demo.mp4), app submitted for review ~1:40am. (4) GitHub secrets TURSO_DATABASE_URL/TURSO_AUTH_TOKEN/BLOB_READ_WRITE_TOKEN were literally "[SENSITIVE]" (session 33 copied masked values) -> fixed from .env.migration.json + Chris's paste; first CI render succeeded and registered social_video:pokemon:set:2026-09-26 (dispatch at 1am posted nothing). MORNING CHECK 09-26 still applies: 6:50am job should say "already registered" and skip; 7:05 post run must show video=yes on the 5 sites + TikTok posted (private). WHEN TIKTOK APPROVES: swap Vercel to production keys, set TIKTOK_PRIVACY=PUBLIC_TO_EVERYONE, Chris flips the account public, delete the duplicate private posts. NEXT after that: Pinterest (Chris's gated task). Uncommitted: src/app/page.tsx max-w-4xl plan-card change (pre-existing, not mine; ask Chris keep/drop) and scripts/mtg-phone.cache.json. PREVIOUS FIRST ACTION (session 34) follows.**

**UPDATE 09-26 ~1am ET (session 34): PRICING LADDER CHANGED + SCAN PACK SHIPPED (Chris 09-25: "one time buy, no subscription, $4.99 for 100 scans"; CardFlip $9.99 → 250 scans; Pro $19.99 → 750; trial 5). ONE SOURCE OF TRUTH: src/lib/pricing.ts (PRICING/PRICE/SCANS/PRICE_LINE/LADDER_SENTENCE); nothing else may hardcode a $ or a scan count (Chris: "remember where all the text changes live"). Pack = users.extra_scans (was dormant), never expires, stacks, spent after month + bonus; no-sub + balance = new ScanTier "pack" (every feature open, sells on eBay). Stripe LIVE (done by API): new Pro price price_1UJmWRHrYyCaAIAxR9eImYvs ($19.99, default on prod_VCLaeDbdIdU2yA, $24.99 price archived, nobody was on it), new product prod_VKRPz96SY78LVp "CardFlip Scan Pack" price price_1UJmWbHrYyCaAIAxvnholZET ($4.99 one-time), old pack products archived, product descriptions say 250/750; Vercel env STRIPE_PRO_PRICE_ID + STRIPE_SCAN_PACK_PRICE_ID updated (prod + preview pro). Checkout: POST /api/billing/checkout {plan:"pack"} → mode=payment, metadata pack=1/packScans; webhook credits via creditScanPack keyed by session id (scan_pack_purchases table, idempotent); welcome page handles ?billing=pack. Tests: test:quota (pack tier + spend order + credit idempotency), test:webhook (pack cases), test:analytics (MRR from PRICING). Pricing page = 4 cards (md 2-col, xl 4-col), verified desktop + phone. NOT DONE / watch: (a) the real first pack purchase on prod — watch Vercel logs for "scan pack +100"; (b) REFERRAL_BONUS_SCANS is still 500 (two months of scans now) — ask Chris; (c) the social post footer does not quote a price (checked). Morning check for the social video (below) still applies.**


**UPDATE 09-26 ~12:30am ET (session 33, later): AUDIO = one royalty-free track (public/social/audio/the_mountain-upbeat-upbeat-music-567448.mp3, Pixabay, 92.5 bpm; Chris hated the synthesized loop, it is deleted). The cut follows the beat (scripts/lib/beat.mjs: tempo/downbeat/loud start; one bar per card, 18.8s for five cards with this track), price is GREEN with glow flare + shine sweep on the pop (Chris: entice, flashier). Chris said 'maybe we will change sound for the next video' - dropping a new MP3 into the folder is all it takes (rotation by day if several). Last push 16021aa. Morning check (a)-(d) below still applies; also LISTEN to the posted video once: beat sync and start point are from an untested-in-the-wild detector.** **FIRST ACTION (saved 09-25 ~11:15pm ET, session 33 end). SOCIAL VIDEO SHIPPED on main (b240698 backing track, 77ffd1b video on all five sites), deployed. Chris 09-25 late: do NOT post videos tonight, save for tomorrow's rotation - so nothing was dispatched. MORNING CHECK 09-26 (first thing): (a) gh run list --workflow social-post.yml: the 6:50am ET ideo job (first ever CI render: npm ci + playwright chromium + social-video.mjs --register --skip-if-done) must be green and print 'registered social_video:pokemon:set:2026-09-26 -> <blob url>'; (b) the 7:05am post run HTTP 200 with posts[].video = 'yes' on every site (or 'fallback' + error = video upload failed, picture went); (c) LOOK at the video on bsky.app/profile/cardflip.bsky.social and /admin/social (Video tab); (d) if the render job failed, the picture went out, fix the job, Run workflow video=1 re-renders (only if Chris says the video can go out that day). Design: docs/SOCIAL-AUTOPILOT.md 'Video' section. Secrets TURSO_DATABASE_URL/TURSO_AUTH_TOKEN/BLOB_READ_WRITE_TOKEN copied to GitHub from Vercel prod. Chris has NOT yet said yea/nay on the backing track (sent him cf-video-audio.mp4). NEXT after the morning check: TikTok (Chris creates account cardflipio + developer app, Content Posting API; unaudited apps post private; I write the audit demo), then Pinterest (his gated task), then movers/drops on the same video engine if he wants. PREVIOUS FIRST ACTION (session 32) follows.****FIRST ACTION (saved 09-25 late, session 32 end). NEXT TASK = SOCIAL VIDEO ON ALL FIVE SITES (Chris 09-25: "pretty good, we should try to incorporate this with the other socials as well, that way we arent always posting static images"; earlier same night he REVERSED the no-video rule: "we are going to have to start making our own video content for tiktok", TikTok is "top of the list"). PROTOTYPE SHIPPED: scripts/social-video.mjs (commit after this save) = setSpotlight() data → 1080x1920 HTML scene → Playwright steps render(t) per frame → ffmpeg-static → H.264 MP4, 15.3s, ~40s to render locally, silent. Countdown No.5→No.1, price counts up, holo progress bar, logo outro. Run: node --experimental-strip-types --no-warnings --conditions=react-server --import ./scripts/lib/register-next-stubs.mjs scripts/social-video.mjs --out x.mp4 [--day]. Local DB has one price point per card so no % lines locally (prod will). PLAN (do in this order, one push each): (1) Backing track: pick a royalty-free loop (public/social/audio/*.mp3, CC0/own license noted in docs), ffmpeg -i audio -shortest, fade out last 0.5s. (2) Render in GitHub Actions, not Vercel (Chromium+ffmpeg do not fit a function): social-post.yml 7am job runs the script with prod DB env (TURSO_* secrets already there?) → uploads MP4 to Vercel Blob (@vercel/blob already used in api/admin/board/image) → calls the publish route with videoUrl. (3) Site adapters, video support: Bluesky app.bsky.embed.video (upload via video.bsky.app with a service auth token, poll job status); X v1.1 media/upload chunked INIT/APPEND/FINALIZE media_category=tweet_video then v2 tweet with media_ids; Threads media_type=VIDEO video_url=<blob url>, poll status, then publish; Facebook /{page-id}/videos file_url=; Instagram media_type=REELS video_url=, poll, publish. All in sites/*.ts behind a `video?: {url, bytes, mime}` field on the post input; picture fallback if a site's video upload fails. (4) Admin /admin/social: show the MP4 with a <video> for the day's set slot before it posts. (5) Slot policy: 7am set spotlight = VIDEO on all five; 1pm movers + 7pm drops stay pictures for now (variety), then movers/drops get the same engine. (6) test:socialvideo covering the timeline math and adapter payload shapes with fetch stubbed. TIKTOK after that: Chris creates the account (cardflipio) + TikTok developer app, Content Posting API; unaudited apps post PRIVATE until audit; I write the audit demo. Memory: cardflip-social-no-video (reversed). Analytics tab: DONE + approved 09-25. PREVIOUS FIRST ACTION (analytics, session 32) follows.**FIRST ACTION (saved 09-25 late, session 32). ANALYTICS TAB SHIPPED (79c354f on main, deployed): /admin/analytics, owner only, phone-first. Ranges 24h/7d/30d/90d (?range=), every number vs the prior period; 12 trend charts (hourly buckets for 24h); funnel (cohort + all-time: signed up → scanned → listed → sold → paying); scanner match rate (cards created / scan_usage calls) + cost per call; scans + price checks by game; top pages, referrers, devices, countries; MRR = $9.99 × active standard + $24.99 × active pro from users.plan/sub_status; social last-post day per site from settings social_last_post:*; help messages + server errors. Range picker = fixed bottom bar on phones. Code: lib/server/analytics.ts (getAnalytics, every query wrapped → zeros never a blank page), app/admin/(console)/analytics/page.tsx, lib/visit.ts (referrerHost, deviceClass), test:analytics. page_views gained ref/device/country columns (COLUMN_PROBES; /api/visit fills them; VisitPing sends document.referrer with the FIRST ping only) — referrer/device/country breakdowns are empty for visits before 09-26, the page says so. Chris 09-25 late: "analytics looks good" (YEA, done). Possible follow-ups only if he asks: Eastern-day buckets instead of UTC, per-user activity table, export. NEXT after his yea/nay = MORNING CHECK 09-26 below, then Pinterest (Chris's gated task), then the social inbox/moderation spec. PREVIOUS FIRST ACTION (session 31) follows.**FIRST ACTION (saved 09-25 ~9:30pm ET, session 31 end, Chris: "save and clear"). NEXT TASK = ADMIN ANALYTICS TAB (Chris 09-25: "make a analytics tab and make it the best you can, nice and detailed ... easier for me if im traveling, to just look at my phone in the admin"). Build it PHONE-FIRST. Step 1 = one Explore subagent (sonnet) to map: admin nav/tabs + role gating, the overview page + its 30-day sparkline tiles (Visitors, Page views, Cards scanned, Sign-ups, Price checks, Sold) and the server functions/SQL behind them, page_views schema + /api/visit + <VisitPing/> (what is and is not recorded per hit), other tables with timestamps (users/plans/trials, scans, price checks, sold, ebay, wishlist, stripe events, referral, social_last_post:*), DESIGN.md rules, existing /api/admin stats endpoints. Step 2 = design: today/7d/30d/90d ranges, daily series with prior-period comparison, funnel (visit → signup → first scan → first listing/sale → paid), top pages + referrers (add columns to page_views if missing: path, referrer host, device class, country from Vercel geo; never raw IP/UA), scans by game + success rate, revenue/subscriptions (Stripe or local), social posts per site, all in the existing tile style, one column on a phone, sticky range picker. Step 3 = ship, then screenshot in mobile emulation for Chris. SESSION 31 SHIPPED (all on main, deployed): token refresh (fb65f93), admin/social Eastern day (13c23ea), 7am SET SPOTLIGHT replaces card of the day (e3fd3ae), QUALITY PASS (13d5918: settled prices/no % on unheld points, $10 floor, gainers vs drops, Star glyph, one-line heading, bigger art), NO-REPEAT rule (3accb82). Chris decided 3/day STAYS ("a good effort at getting some volume"). MORNING CHECK 09-26: (a) 7am run report (gh run list --workflow social-post.yml) HTTP 200, set spotlight posted on all 5 sites, LOOK at the picture on bsky.app/profile/cardflip.bsky.social: % lines present, no boxes in names, footer on the picture; (b) dip email (below); (c) Facebook OAuth error = Chris re-pastes an extended token (below). Chris has not yet given yea/nay on the new post look. Pinterest business account = Chris's one gated task (then I build one-click OAuth). Local dev: .env.local has CRON_SECRET=local-dev-only (untracked) for rendering /api/social/image locally; local price_series has one point per card so local renders show no % lines and no movers. Memory added: cardflip-social-quality-bar. PREVIOUS FIRST ACTION (session 30) follows. UPDATE 09-25 ~8:15pm ET (session 31): TOKEN REFRESH DONE (fb65f93): Instagram Login + Threads 60-day tokens renew weekly from the publish route (refreshMetaTokens in sites/meta.ts, settings social_token:<site>, test:socialpublish covers it); the "build a refresh step before ~11-20/11-24" items below are CLOSED. Next = Pinterest (Chris makes the account, one gated task), then the social inbox/moderation spec. 7AM POST IS NOW THE SET SPOTLIGHT (Chris 09-25 ~8:30pm: "i dont really like these posts about 1 card, i like more informative posts with multiple cards"): setSpotlight() in lib/server/social.ts = five priciest cards of one set (date-hash over sets with >=5 cards >= $2), image ?kind=set (Movers renderer, mode="price"), SLOTS.morning kind "set"; card of the day code kept but unposted. Chris has NOT yet said yea/nay on the look: check /admin/social with him first. QUALITY PASS SHIPPED same evening (Chris twice: "highest quality possible", images AND text): settled-price rule (held 3/7 days else week median, no % claimed), week-ago price must be a real point (<=3 day carry), MOVER_MIN_PRICE $3 -> $10, 1pm gainers-only / 7pm drops-only, star glyph -> "Star", one-line set heading, bigger art; rules + how to render locally are in docs/SOCIAL-AUTOPILOT.md "QUALITY RULES". Memory: cardflip-social-quality-bar. Local price_series has one point per card, so local renders show no % lines and no movers; judge % lines on prod after the 09-26 7am post. Also fixed: /admin/social defaulted to the UTC day (jumped to tomorrow after 8pm ET), now the Eastern day. STANDING REMINDER, ONCE A DAY FIRST REPLY ONLY (Chris 09-25 hates nagging): iPostal1 mailbox personal → business plan before it renews Oct 5. MORNING CHECKLIST: (1) DIP EMAIL: did the wishlist dip email reach truefreemoney+dip402@gmail.com after the 09-26 09:45Z pokemon-prices cron? yes → tick the board row + delete the Dip Test user; no → vercel logs, result.wishlistAlerts. (2) META: THREADS LIVE 09-25 ~7:15pm ET (appeal won; Meta developer app "CardFlip" with the Threads use case, cardflipio = Threads tester, THREADS_TOKEN pasted by Chris into Vercel prod, first post https://www.threads.com/@cardflipio/post/DdunR86mu_M; THREADS_USER_ID not needed, /me). Long-lived Threads tokens last 60 days: refresh before ~11-24 (GET graph.threads.net/refresh_access_token?grant_type=th_refresh_token) — add a refresh step to the publisher before then. FACEBOOK LIVE 09-25 ~8:15pm ET: Chris pasted a USER token from the Graph API Explorer (use case "Manage everything on your Page", perms pages_show_list/manage_posts/read_engagement) as META_PAGE_TOKEN; the adapter swaps it for the Page token via /me/accounts (first post https://www.facebook.com/1314676718400499_122107444059482847, Page id 1314676718400499). UNKNOWN whether Chris extended the user token (1h vs 60d): if the 7am 09-26 Facebook post fails with an OAuth error, he redoes Explorer → Generate → ⓘ → Access Token Tool → Extend → paste. INSTAGRAM LIVE 09-25 ~9pm ET: use case "Instagram API with Instagram Login" on the same app (Instagram app id 1080258094857695, cardflipio = Instagram tester, professional account), INSTAGRAM_TOKEN on Vercel prod, first post https://www.instagram.com/p/DdurlqVFkiz/ (adapter: graph.instagram.com, /me). Instagram Login + Threads tokens are 60-day long-lived: build a refresh step (GET /refresh_access_token?grant_type=ig_refresh_token | th_refresh_token) before ~11-20. ALL FIVE SOCIAL SITES LIVE: Bluesky, X, Threads, Facebook, Instagram, 3/day hands off; next = Pinterest, then the social inbox/moderation spec. INDIA BLOCKED site-wide 09-25 (src/proxy.ts BLOCKED_COUNTRIES default IN, provider callbacks exempt, test:admingeo). Facebook Page ↔ Instagram link: RESTRICTED again 09-25 (new-account restriction), parked a few days; Facebook + Instagram can each go live WITHOUT the link (Page token via the same developer app; Instagram via "Instagram API with Instagram Login", needs a small adapter variant on graph.instagram.com). Chris: delete the "Threads token" note from his thoughts (I could not: 5-min admin idle + classifier blocks reading secrets off the board). Adapter: sites/meta.ts (facebook/instagram/threads). (3) Chris to verify on his phone that scanning a PSA slab gets the refusal (613c14f). SESSION 30 SHIPPED: X LIVE (@cardflipio, keys on Vercel, x.ts adapter, first posts confirmed; console.x.com pay-per-use $0.015/post, $5 credits ≈ 330 posts, auto-recharge off → 402 = out of credits, tell Chris to top up). SOCIAL ON X + BLUESKY = 3/day hands off. DAILY VISITORS + PAGE VIEWS tiles on the admin overview (page_views table, POST /api/visit from <VisitPing/>, daily-changing hash, /admin never counted). GOOGLE ANALYTICS 4 LIVE (NEXT_PUBLIC_GA_ID on Vercel prod, property CardFlip under account cardflipio, public pages only; reports fill in 24-48h). Privacy page names both. PARKED UNTIL META CONNECTS (Chris ok 09-25): SOCIAL INBOX + MODERATION, spec on the board row under Claude (daily digest line, approve-to-reply, hide sellers/spam on sight, block on 2nd offence, borderline → digest). Other holds unchanged: soft-launch invites, Lorcana + Magic, graded slabs locked (PSA pricing pending), Magic still publicly on. GOTCHAS THIS SESSION: PowerShell Get-Content/WriteAllText double-encoded UTF-8 accents (use node or the Edit tool for files with accents); npx prettier is NOT a project dep and reformats whole files (do not run it); board-read reply lines start with ↳ and the window must stop at the next '- [' line or you read the next row's secret.**

**PREVIOUS (session 29 save, 09-25 ~5:45pm ET, session 29 end, Chris: "save and /clear"). PARKED (Chris ok 09-25): SOCIAL INBOX = daily comments/replies digest on the board with approve-to-reply, build AFTER Meta connects (board row under Claude has the spec). UPDATE 09-25 ~7pm ET: DAILY VISITORS on the admin overview (page_views table, POST /api/visit from <VisitPing/> in the root layout, hash changes daily, /admin never counted; tiles Visitors + Page views). GOOGLE ANALYTICS wired (<GoogleAnalytics/>, NEXT_PUBLIC_GA_ID, public pages only) LIVE 09-25 ~7:30pm ET (NEXT_PUBLIC_GA_ID on Vercel prod, property CardFlip under account cardflipio, stream cardflip.io). Privacy page already names both. UPDATE 09-25 ~6pm ET: X IS LIVE (@cardflipio, four X_* keys on Vercel prod, adapter src/lib/server/sites/x.ts, first two posts confirmed on x.com/cardflipio; console.x.com is pay-per-use $0.015/post, Chris bought $5 = ~330 posts, auto-recharge off; watch for 402 = out of credits). Item (3) X below is DONE, skip it. STANDING REMINDERS ARE ONCE A DAY, FIRST REPLY ONLY (Chris 09-25). OPEN WITH THE STANDING REMINDER: iPostal1 mailbox personal → business plan before it renews Oct 5 (board row under Chris; say it ONCE A DAY, first reply only (Chris 09-25), until ticked). THEN THE MORNING CHECKLIST, in order: (1) DIP EMAIL: ask Chris whether the wishlist dip email reached truefreemoney+dip402@gmail.com after the 09-26 09:45Z pokemon-prices cron (test trial account "Dip Test" watches sv04.5-054 at $9,999). Yes → tick the board row, delete the Dip Test user (admin console → users, Chris signs in), done. No → vercel logs for the cron run, result.wishlistAlerts. (2) META: threads.net appeal result (selfie submitted 09-25 ~5:30pm; a denial = that Instagram is permanently gone, make a new one from his phone with his personal cell); then retry ONCE Facebook Page CardFlip → Settings → Linked accounts → Instagram → Connect account (login cardflipio). When connected → write up step 2 (Meta developer app → Page token + IG business id + Threads token; adapter src/lib/server/sites/meta.ts NOT yet written — write it while he clicks). (3) X: Chris may have made the account (support@cardflip.io, handle cardflip/cardflipio); step 2 = developer.x.com free tier, OAuth 1.0a user keys (4 values) pasted on his board row; adapter src/lib/server/sites/x.ts not yet written. (4) Chris to verify on his phone that scanning a PSA slab now gets the refusal (613c14f). SOCIAL AUTOPILOT IS LIVE: 3 posts/day 7am/1pm/7pm ET, Pokémon only, GitHub cron social-post.yml, catch-up rule, Bluesky @cardflip.bsky.social posted twice 09-25 (card + movers; the 7pm drops post fires 23:05Z, check the board Completed line / gh run list --workflow social-post.yml). Every future platform gets the same posts at the same times automatically. Trial scans = 5 everywhere (9cda1a5). Graded slabs LOCKED (PSA free tier gone; unlock = lib/grading.ts GRADED_LOCKED). Soft-launch invites ON HOLD (Chris). Lorcana + Magic on hold; Magic is still publicly switched on (Chris has not answered whether to flip magic_public off). PSA: Chris to forward their reply with 500/1,000-per-day pricing.**

**PREVIOUS (session 29 detail). STANDING REMINDER UNTIL TICKED ON THE BOARD: tell Chris once a day, first reply only, until Oct 4 that the iPostal1 mailbox must move from the personal plan to business (Mailbox Settings → Change Plan) before the personal plan renews Oct 5. Live board row under Chris (added 09-25 via board-add). SECOND-DEVICE WALK SKIPPED (Chris has no Android). PROVEN ON PROD 09-25: trial-selling 402 gate (fresh trial account "Dip Test" truefreemoney+dip402@gmail.com, password in session scratchpad only; draft + publish both 402 {paywall,selling}). WISHLIST DIP EMAIL ARMED, NOT YET SEEN: that account watches sv04.5-054 (Charizard ex Paldean Fates, USD holo $5.60, price_series updates daily) with alert $9,999 → the 09-26 09:45 UTC pokemon-prices cron (runDailyIfDue → sweepWishlistAlerts) must email truefreemoney+dip402@gmail.com, which lands in Chris's Gmail. ASK CHRIS: did the dip email arrive (subject from sendWishlistAlertEmail in lib/server/mail.ts)? If yes → tick the BOARD row, delete the Dip Test user (admin console → users, or ask Chris), move on to soft-launch cohort measurement. If no → check Vercel logs for the 09:45Z cron run and wishlistAlerts in the response (result.wishlistAlerts = {checked, sent}); a failed send stamps nothing and retries next day. Board rows updated in this commit. SOFT-LAUNCH INVITES ON HOLD (Chris, 09-25 late: "hold on the invites"; cohort ledger has 0 strangers, nothing to measure until he says go). TRIAL SCANS CUT 10 → 5 everywhere (9cda1a5, Chris 09-25). BLUESKY DONE 09-25: account @cardflip.bsky.social (did:plc:yfkyq35rrwr2xzdu7bmejlup, login verified with the app password from Chris's board reply), BLUESKY_HANDLE + BLUESKY_APP_PASSWORD on Vercel production (bypass mode), redeploy = this commit. SOCIAL IS NOW 3 POSTS/DAY, HANDS OFF (Chris 09-25): slots in lib/server/socialPublish.ts SLOTS (7am card / 1pm movers / 7pm dips = new PostKind "dips" = biggest drops, Eastern via Intl), guard key social_slot:<site>:<slot> = ET day, schedule = .github/workflows/social-post.yml (6 UTC crons = each slot at its EDT + EST hour, second ping no-ops; Run workflow = manual with slot/force inputs), auth = SOCIAL_POST_KEY (Vercel prod + GitHub secret, generated by me; CRON_SECRET still works). Pokémon only (577d99a, socialGames()). CATCH-UP RULE (Chris 09-25, "same initial post for all platforms"): each run posts every past slot the site has not posted today, so a new platform gets the day's earlier posts on connect; a slot with no draft stays quiet (no repeats). Post now = re-does the latest due slot when nothing is owed. FIRST POST LIVE 09-25 ~3:55pm ET: Friend Ball Skyridge 126 at bsky.app/profile/cardflip.bsky.social/post/3mwekaz7apv2r (first try 401: PowerShell piped a newline into the env values; re-added with printf, adapter trims). NOTE: test:socialpublish silently crashed on 70939f0 (const at clash) and was fixed in the catch-up commit. The old Tue/Thu/Sat gating is gone; the pokemon-prices cron still calls publishSocial at 5:45am ET and now just reports "outside the windows". CHECK NEXT SESSION: gh run list --workflow social-post.yml → each run HTTP 200 with a posted line; posts at bsky.app/profile/cardflip.bsky.social; Completed lines on the board. Known: Satori logs "Failed to load dynamic font for ★" on /api/social/image (harmless, image still renders). Optional later: domain handle @cardflip.io via a DNS TXT record _atproto.cardflip.io = did=did:plc:yfkyq35rrwr2xzdu7bmejlup, then Bluesky Settings → Handle → own domain. THEN: Meta (IG/FB/Threads) per docs/SOCIAL-AUTOPILOT.md §3. Dip email check is still open for 09-26 morning. BUSINESS PHONE (confirmed by Chris 09-25): +1 443-372-8179 is a Google Voice number under his Gmail; it is the Stripe public support phone. Google Voice forwards calls to his cell and emails voicemail transcripts per its Settings (he checks there); never put a personal cell on Stripe. Do not put the number on Facebook/Meta (no WhatsApp, no Call now); support = support@cardflip.io. Facebook Page action button = Sign up → cardflip.io/signup. GRADED SLABS LOCKED 09-25 (PSA email 09-16: free tier gone, that was the 403 all along, not Vercel IPs): lib/grading.ts GRADED_LOCKED; CARD_READ_SCHEMA gained "slab" (boolean), scanner refuses with SLAB_MESSAGE, CardEditor hides the Graded select unless the item already has a grade, /api/psa/cert 503, help #graded first paragraph. Unlock when Chris buys a PSA plan (prices requested). META RESTRICTED 09-25 ~5pm ET ("Your account is restricted … try again later" on Page → Linked accounts → Instagram → Connect account; new-account cooldown, Google Voice number was rejected for IG SMS verification = VoIP block): remind Chris to retry ONCE after Sep 26 5pm ET (board row under Chris; session cron 4e2a2e52 dies on /clear). THREADS/IG APPEAL: Chris uploaded the selfie 09-25 ~5:30pm ET, Threads shows "You submitted an appeal on September 25, 2026" (about an hour to review; if Meta says the account does not follow Community Standards it is permanently disabled, no second appeal). Check threads.net first tomorrow, then the Page link. Instagram @cardflipio exists (business account, in Accounts Center with support@cardflip.io); Page ↔ IG link NOT yet made; Threads not yet made. META ACCOUNTS IN PROGRESS: Facebook Page CardFlip made 09-25 (cover public/brand/cardflip-cover-1640x624.png, website + support email set, no phone/WhatsApp); Instagram signup as cardflipio with support@cardflip.io was on screen at save; Threads next; then the developer app + tokens (step 2, not yet written up).**

**PREVIOUS (saved 09-25, session 28). LORCANA + MAGIC ON HOLD (Chris). FOCUS = site ready BEFORE social accounts. DONE 09-25: Stripe public details (CardFlip / iPostal1 Chevy Chase / CARDFLIP) verified via live API. INCIDENT FIXED 09-25: Vercel production STRIPE_SECRET_KEY had been the SANDBOX sk_test since 09-02 11:13Z (flip-stripe-live.mjs read .env.local STRIPE_LIVE_SECRET_KEY, which is the sandbox); live key from stripe2.txt restored + prod redeployed (dpl_GCwzYbLgSXAnj8GJHeH4TMAUb6BQ), env pull confirms sk_live -> CardFlip; script now reads stripe2.txt (c45e30a). Secret writes need Chris to click bypass permissions. DONE 09-25 later: paid signup gate PASSED end to end (promo CHRISTEST, $0 invoice, webhook, DB active); prod mail DELIVERS (SMTP test + new signup welcome email, 5fe88f0, both landed in Gmail); signup step 2 is now a welcome card, eBay behind a link (3264e4d); checkout allows promo codes (13fc3ba). Subscription email confirmed; post-checkout landing /app/account/welcome + Cancel plan flow shipped, Chris yea. Real-phone walk on prod PASSED 09-25 (Chris 'all good': camera/torch, scan, draft, Connect eBay, publish). Stripe branding + public details all saved. NEXT, in order: (1) second device (Android or a friend's iPhone) walk of the same five steps. (2) trial-selling 402 gate + wishlist-dip email proven on prod. (3) soft-launch cohort measured. Then social accounts (docs/SOCIAL.md, Bluesky first). Then social (docs/SOCIAL.md).

**PREVIOUS (saved 09-17, session 27 end, Chris: "save, reset, and continue with lets go").** ADMIN CONSOLE IS FIXED AND VERIFIED: Chris signed in at 23:35 on the fix deployment (4966662) and every tab rendered in the same second. Cause + fix in PREVIOUS. TOOLING RULE from 09-16: use the PowerShell tool for commands on this box, not Bash (Bash strips backslashes; PowerShell 5.1 syntax, no && chaining). OPEN ITEMS, in order: (1) LORCANA SWITCH = Chris's yea/nay — panel 145/145 = 100%, eBay seller photos 38/38 = 100%, CI + smoke green; flip = INSERT settings lorcana_public=1 on prod via .env.migration.json exactly like onepiece_public (set 09-10); then all four games are public and the 98% accuracy push is done. (2) Forgot-password flow verified wired (login → /forgot-password → SMTP via Fastmail from support@cardflip.io, prod returns ok) but no real delivery ever observed — offer Chris a test email to his own address. (3) One Piece scans cost ~4¢ (tiebreak on 85% of scans, base/reprint twins) vs 2¢/scan revenue at $9.99/500 — a follow-up if One Piece volume shows up; Lorcana/Pokémon ~1.4¢. (4) Back to the board after that. Board runner note for Chris: Tasks-page Run uses his claude.ai subscription (cloud routine, Fable 5.1), not the metered API key; only scans/tiebreak/help chat use the API key.
