...
**FIRST ACTION (saved 09-25, session 28). LORCANA + MAGIC ON HOLD (Chris). FOCUS = site ready BEFORE social accounts. DONE 09-25: Stripe public details (CardFlip / iPostal1 Chevy Chase / CARDFLIP) verified via live API. INCIDENT FIXED 09-25: Vercel production STRIPE_SECRET_KEY had been the SANDBOX sk_test since 09-02 11:13Z (flip-stripe-live.mjs read .env.local STRIPE_LIVE_SECRET_KEY, which is the sandbox); live key from stripe2.txt restored + prod redeployed (dpl_GCwzYbLgSXAnj8GJHeH4TMAUb6BQ), env pull confirms sk_live -> CardFlip; script now reads stripe2.txt (c45e30a). Secret writes need Chris to click bypass permissions. NEXT, in order: (1) Chris's own paid signup on prod, trial -> wall -> Stripe -> webhook; offer a 100%-off live promo code first so nothing needs refunding. (2) forgot-password test email to truefreemoney@gmail.com. (3) welcome email / wishlist-dip / trial-selling 402 proven on prod. (4) real-phone walk + a second device. Then social (docs/SOCIAL.md).
