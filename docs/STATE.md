...
Last updated: 2026-09-06 evening (session 11; resume = this file only; FIRST ACTION block = "Start here next session", read with `limit: 90`).
...
**FIRST ACTION (saved 09-06 evening, session 11 in progress). v1.0.0 IS TAGGED (09-05, 262462e): all launch gates closed. Chris's numbered list is DONE through 6 (1 Magic off, 2 Anthropic auto-reload, 3 eBay Growth Check filed — eBay acknowledged 09-06 12:43am, 1-2 business days, 4 IQ-85 pass YEA, 5 Fastmail rule support@ → folder "CardFlip Support", 6 cdemon Plan back to Automatic). SESSION 11 SHIPPED (all main, deployed): admin Plan dropdown widened (626665b); daily error digest — lib/server/errorDigest.ts at the end of /api/cron/pokemon-prices (+ /daily) mails OWNER_EMAIL when error_events > ERROR_DIGEST_MIN (5) in 24h, test:errordigest; eBay Browse comps cached per card per day — lib/server/ebayCompsCache.ts on card_cache under ebaycomps:v1:, test:compscache (02c5a03). ALSO SHIPPED: test:adminroutes (45 checks, next/headers+next/cache stubbed via scripts/lib/register-next-stubs.mjs), test:ebaysync (29 checks: orders→sold, ended listings, fees over a fetch stub), docs/SOFT-LAUNCH.md (the one-pager — Chris to read). ALSO: test:ebaysweeps (29) + test:ebaysell (29) — the eBay libs are now covered end to end over fetch stubs (only daily-job orchestration + price refreshes left, need network fixtures). SHIPPED LATE 09-06 (awaiting yea/nay): INVITE A FRIEND / REWARDS — lib/server/referrals.ts (codes, attach at signup via ?ref= → RefCapture.tsx localStorage → signup body.ref, rewardReferrerIfDue on the Stripe subscribed edge: +500 bonus_scans to a SUBSCRIBED referrer, once per friend), scanQuota spends bonus after the monthly cap, /app/rewards page (holo headline, link + Copy/Share, tally, 3 steps), header link "Unlock 500 free scans" (subscribers only), account row; test:referral (24). Verified locally in mobile emulation. HAIKU A/B DONE 09-06 (191 prod photos, backups/ab-vision-0906-haiku.json): Haiku 4.5 is WORSE and SLOWER — number correct 109/191 vs Sonnet 155/191, name 172 vs 187, latency 4.4s vs 3.7s; only cheaper (/usr/bin/bash.0042 vs /usr/bin/bash.0115/scan). Sonnet stays. Do not re-run unless a new Haiku ships. PERF 09-06 (Chris: "site locking up, slow loading", stage sometimes empty): /api/cards/featured measured 12s cold on prod — it called pokemontcg.io (2 queries × 5 retries × 8s) per cold function; now lib/server/stageCards.ts = mirror-first (searchEnglishCardsLocal × 10 icons + latestUsdPrices batch) cached 6h in card_cache (stage:v1:*), 0.44s miss / 11ms hit locally. ALSO every cold start ran SCHEMA + 46 ALTER probes against Turso; db.ts now stores a sha1 fingerprint of SCHEMA+COLUMN_PROBES in price_history_meta and skips the pass when unchanged (one SELECT). If slowness persists: check Vercel function region vs Turso aws-us-east-1, and `npx vercel logs cardflip.io` (CLI is logged in). MY QUEUE NEXT: Playwright E2E (tour/robot/CardTile/SetBrowser/stage) — only if Chris wants the dependency; NEED CHRIS: Haiku scanner A/B (prod ANTHROPIC key), off-site backup destination (Drive/S3). CHRIS NEXT TASK (parked 09-06 "save for later"): invite the FIRST soft-launch seller — one person he knows who sells Pokémon singles on eBay by hand; message text is in docs/SOFT-LAUNCH.md "Who to invite"; then tell me who so I watch their account. Test accounts STAY (Chris: "i want my test accounts"). Chris-side open after that: eBay live-test batch, MTG stress test, MD LLC decision. BACKLOG §0.0 is the one list.** SESSION 10 SHIPPED (all main, deployed): paywall one sentence/one button; editor = "Is this your card?" before Verify then FULL sheet (Chris rejected hiding it behind Change); scan chip "Check and publish"; help panel portalled to body (header backdrop-blur pinned the fixed sheet on iPhone); Magic switch busts static cache (revalidatePath layout) — public pages incl. /help /terms /privacy /app/* build STATIC and read the switch at build; terms/privacy/footer follow the switch; docs/ARCHITECTURE.md (Phase 0, baseline all green, fly.toml+Dockerfile deleted); test:helpchat (fake Anthropic server) + test:webhook (fetch stub; fixed: update event w/o price no longer demotes Pro); remembered scan category keyed per user + cleared on logout (was leaking across accounts in one browser); SUBSCRIBE-FIRST for trial sellers: publish row / connect card / queue summary / header pill all say "Subscribe now" → /pricing; PlanCta.tsx makes plan cards live when signed in (checkout direct; "Your current plan"; portal for switch); RULE LEARNED TWICE: gate on user.tier (server-resolved, honours admin overrides), NEVER raw subStatus — trialScansLeft + Paywall + PlanCta fixed; billing/checkout still 409s a real Stripe subscriber under a Trial override (correct — test purchases need a fresh account). Remote Control is on for this session. BACKLOG §0.0 is the one list (36 boxes; 10 are passive prove-on-prod).**
