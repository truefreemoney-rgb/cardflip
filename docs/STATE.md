...
**ARCHITECTURE (09-05): `docs/ARCHITECTURE.md` is the one-screen inventory — what is live / legacy / recovery path / ranked risks / baseline. Read it instead of grepping this file for infra facts. Phase 0 of the stewardship brief is DONE; later phases (migrations out of the request path, jobs off Vercel functions, scanner queue loop into a module) are argued one at a time on evidence, not run as a plan.**
