...
**FIRST ACTION on "lets go" (saved 08-25 evening, Chris: "save, clear and resume"):**
**YOU ARE MID-MIGRATION (Fly → Vercel), working tree is on branch `vercel-migration`** — read `docs/MIGRATION.md` (~120 lines) as your second read, it has full phase status + gotchas. Where things stand: Phase 1 (async DB on @libsql/client) DONE+verified; Turso provisioned & seeded (330k rows); Vercel preview LIVE and smoke-passed (https://cardflip-q6yvf86s6-card-flip1.vercel.app, project cardflip, team card-flip1); Phase 2 (photos→Tigris) BARELY STARTED — only `getObject`/`s3Configured` added to `backup.ts` (committed, unfinished); Chris said "do what you can" then stopped me for this reset, so CONTINUE Phase 2 without asking: (a) `cardPhotos.ts` S3 backend behind `s3Configured()` (keys `photos/<id>.jpg`, fs fallback for dev), make `deleteCardPhoto` async + await its 3 callers, (b) `scripts/migrate-photos.mjs` for cutover, (c) Phase 3: cron routes (`/api/cron/mtg-prices`, `pokemon-prices` — fold ebay-sales into it, maybe `sweep`), `export const maxDuration = 300`, auth = existing CRON_SECRET Bearer pattern (Vercel auto-sends it when the env var exists), `vercel.json` with **max 2 crons (Hobby plan limit, daily only)**, guard the heartbeat/instrumentation daily-job triggers with `if (!process.env.VERCEL)`, (d) set AWS_*/BUCKET_NAME + a fresh random CRON_SECRET on Vercel env via API, redeploy branch, smoke incl. photo PUT/GET round-trip. **Creds: `.env.migration.json` in repo root (gitignored)** — turso db url/token, turso platform token, vercel token; team_l73XXJDNrLnYFezMhk8bn2K2 / prj_GlXQxal5IAh2oG0zls2ZGjhK212b. Tigris AWS keys are NOT in it (classifier blocked writing them; they're Fly secrets — printed in the 08-25 morning transcript, or mint new keys from Fly dashboard → Tigris). Vercel API pattern that works: PowerShell Invoke-RestMethod, Bearer token, `?teamId=`; NEVER name a variable `$pid` (reserved); classifier blocks reading env values back (decrypt) — don't try. Deploy trigger: POST v13/deployments {name:"cardflip", gitSource:{type:"github", org:"truefreemoney-rgb", repo:"cardflip", ref:"vercel-migration"}}. Fly prod (main branch) STAYS live at cardflip.io throughout — do NOT merge the branch to main. Chris items at cutover (not before): 3 secrets from dashboards, 2 eBay-portal URLs, DNS. Also parked: logo (3 concepts shown 08-25, he hasn't picked; "the strike" recommended).
