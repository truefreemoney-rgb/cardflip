...
**FIRST ACTION (saved 09-06 ~3am ET, session 11 end — Chris /cleared mid-task). RESUME = MOBILE QA SWEEP, Chris: "mobile has a ton of issues… deploy the subagents and yourself, do like 100 simulations and try to debug everything… test for apple and android." Nothing of it landed yet (two audit subagents were launched and died with the session). DO THIS: (1) launch two background general-purpose agents, read-only: A = static mobile-UX audit of src/app + src/components + globals.css (hover-only affordances, tap targets <40px, inputs <16px → iOS zoom, window.confirm/alert, 100vh/h-screen, fixed-inside-transform, overflow-x risks, safe-area insets, file/camera inputs, z-index vs sticky header z-40); B = client robustness audit (fetches without timeout/catch, stuck loading states, uncleaned streams/timers/rAF, camera getUserMedia constraints + playsInline + backgrounding, object URLs never revoked, iOS standalone PWA quirks incl. OAuth returns + localStorage try/catch, Android keyboard/visualViewport, manifest/viewport meta). Each returns ≤40 items P1→P3 with file:line. (2) Meanwhile drive the dev server (preview_start cardflip-dev; the pane is usually hidden → use javascript_tool/read_page, not clicks) in mobile emulation at 375x812 AND 360x780 (resize_window custom) as anon, trial, subscribed (local account invite-test@example.com / hunter22, sub_status active in data/cardflip.db), and admin (Log In as Admin button), over / /login /signup /pricing /help /app /app/collection /app/price-check /app/wishlist /app/account /app/rewards + open the editor, the help panel, the tour (?tour=1), the paywall, the camera sheet. Per page run one JS audit: document.documentElement.scrollWidth>clientWidth, elements wider than viewport, interactive elements with rect <40px, console errors, failed network calls, text clipped by overflow hidden, fixed elements overlapping the header. That is the ~100 simulations. (3) Fix real defects in batches, tsc+eslint+relevant test scripts, push HEAD:main, keep a written list in docs/BACKLOG.md §0.0 under a new "Mobile QA 09-06" heading (found / fixed / left). Chris judges on his iPhone afterwards. CONTEXT FROM TODAY (all on main, deployed, yea'd): Invite a friend / Rewards (lib/server/referrals.ts, /app/rewards, header pill "✦ Unlock 500 Free Scans" for owner+subscribers, help robot moved to the right of the header), Title Case sweep on all buttons/links/tabs (rule in docs/DESIGN.md → Casing), perf fix (stage cards from mirror + 6h cache; schema-fingerprint gate in db.ts — cold starts no longer run 46 ALTERs), Haiku A/B = NO (Sonnet stays), tests added: errordigest, compscache, adminroutes, ebaysync, ebaysweeps, ebaysell, referral. Vercel: Fluid Compute on, region iad1 (Chris confirmed defaults). CHRIS-SIDE PARKED: first soft-launch invite (docs/SOFT-LAUNCH.md), off-site backup destination. SESSION 10 SHIPPED (all main, deployed): paywall one sentence/one button; editor = "Is this your card?" before Verify then FULL sheet (Chris rejected hiding it behind Change); scan chip "Check and publish"; help panel portalled to body (header backdrop-blur pinned the fixed sheet on iPhone); Magic switch busts static cache (revalidatePath layout) — public pages incl. /help /terms /privacy /app/* build STATIC and read the switch at build; terms/privacy/footer follow the switch; docs/ARCHITECTURE.md (Phase 0, baseline all green, fly.toml+Dockerfile deleted); test:helpchat (fake Anthropic server) + test:webhook (fetch stub; fixed: update event w/o price no longer demotes Pro); remembered scan category keyed per user + cleared on logout (was leaking across accounts in one browser); SUBSCRIBE-FIRST for trial sellers: publish row / connect card / queue summary / header pill all say "Subscribe now" → /pricing; PlanCta.tsx makes plan cards live when signed in (checkout direct; "Your current plan"; portal for switch); RULE LEARNED TWICE: gate on user.tier (server-resolved, honours admin overrides), NEVER raw subStatus — trialScansLeft + Paywall + PlanCta fixed; billing/checkout still 409s a real Stripe subscriber under a Trial override (correct — test purchases need a fresh account). Remote Control is on for this session. BACKLOG §0.0 is the one list (36 boxes; 10 are passive prove-on-prod).**
