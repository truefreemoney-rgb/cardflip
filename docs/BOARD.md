...
- [ ] [Claude] RUNNER UPGRADE 4 — let it see prod, safely: a read-only endpoint on cardflip.io (last errors, last prod smoke, scan usage) behind its own secret, handed to the routine as its one env var. NOT a Vercel token (full-account). WAITING on Chris for a yes. ~1h.
