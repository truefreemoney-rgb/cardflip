...
- [x] 09-09 BOARD RUNNER LIVE: ▶ Run → GitHub issue (label board-run) → cloud routine "CardFlip board runner" (Sonnet 5, fires on issue opened/labeled + daily 6am ET) → PR "Closes #n", never merges, labels needs-chris with questions when unsure. Proven on issue #1 (runner asked questions; Chris scrapped the idea). RUNNING #n on the board links to the issue. Board is one-way for now: outcomes arrive as GitHub emails.
