...
- [ ] [Claude] SOCIAL, HANDS-OFF. Step 1 shipped 09-10 (922f30d): content engine + /admin/social (movers of the week, card of the day, pictures at every site size, caption + Copy). Step 2 shipped 09-10: the publisher — posts every draft to every connected site Tue/Thu/Sat with the daily cron, once a day, and leaves a line here under Completed; /admin/social now shows each site (connected / last post) and a Post now button. Bluesky adapter ready — waiting on your app password (your row above). Image sites only (Chris: we are not video creators). Next: Meta (IG/FB/Threads) → X → Pinterest. Chris only creates each account and pastes one token per row. Plan: docs/SOCIAL-AUTOPILOT.md.
