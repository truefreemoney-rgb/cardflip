...
- [ ] [Claude] MAGIC LOCKDOWN — PUBLIC since 09-10 midday (panel 99.0%, phone 42/42 after 3 ledger fixes, CI+smoke green). (Chris 09-10: "nail this down like Pokémon"; his cards are 2025+): research → docs/MTG-IDENTIFICATION.md. PHASE 1 SHIPPED 09-10: the Magic scan has its own read (MTG_READ_SCHEMA: finish, treatment, marks, artist, copyright year, border, serial), the mirror stores artist/frame/border/frame_effects/promo_types/full_art/textless, the ranker breaks ties on them (cuePenalty; The List icon + original code/number lands on PLST; as-is reprint sets skip the year cue), the scan's finish is the price line, Magic photos upload at 1568px (List icon invisible at 1024). Catalog-image replay: 5/5 exact printing. Prod cue data pushed 09-10 (94,144 printings). CHRIS'S 10-CARD BATCH 09-10 (Spider-Man/Marvel, all 2025): 10/10 exact printing (set code + number read every time, incl. the #2xx extended/showcase variants); finish read foil on 7, nonfoil on 2, unsure on 1 — awaiting Chris's foil truth; treatment read disagreed with Scryfall on 6/10 (Marvel frames read as showcase, extended-art read as standard) but never mattered because the number pinned it. NEXT: Chris's foil answer → tune the finish prompt if wrong; then phase 2 panel. Phase 2 = replay-scans mtg panel (≥95% clean) then Chris's 30-card phone batch (≥90%). Phase 3 = pick-by-picture on ties. Magic stays admins-only until the numbers pass. FLAVOR_NAME DONE 09-10 (Shards of Narsil → Thorn of Amethyst; npm run mtg:flavor; prod via push-mtg-cues).
...
- [x] 09-25 iPostal1 business name — NOT NEEDED (no LLC; Stripe public business name = CardFlip is enough). Was: Mailbox Settings → Change Plan → Virtual Business Address (from $14.99/mo); do it with the LLC (ticket #3081502).
- [x] 09-25 Stripe public details errand DONE + verified via live API: name CardFlip, iPostal1 address (Chevy Chase MD), support@cardflip.io, descriptor CARDFLIP. FOUND+FIXED same day: prod STRIPE_SECRET_KEY had been the SANDBOX key since 09-02 11:13Z (flip script read the wrong .env.local line); live key restored on Vercel production + redeployed, script now reads stripe2.txt. v1.0.0 tag now waits only on Chris's own paid signup.
...
- [ ] [Claude] SOCIAL, HANDS-OFF. Step 1 shipped 09-10 (922f30d): content engine + /admin/social (movers of the week, card of the day, pictures at every site size, caption + Copy). Step 2 shipped 09-10: the publisher — posts every draft to every connected site Tue/Thu/Sat with the daily cron, once a day, and leaves a line here under Completed; /admin/social now shows each site (connected / last post) and a Post now button. BLUESKY LIVE 09-25: @cardflip.bsky.social (avatar public/brand/cardflip-avatar.png). THREE POSTS A DAY, hands off (Chris 09-25): 7am card of the day, 1pm movers, 7pm price drops (Eastern), Pokémon only, fired by .github/workflows/social-post.yml (GitHub cron, key SOCIAL_POST_KEY on Vercel + GitHub) → POST /api/social/publish?slot=; each slot once per ET day; every post leaves a Completed line here. Image sites only (Chris: we are not video creators). X LIVE 09-25 (@cardflipio, pay-per-use, $5 credits). Next: Meta (IG/FB/Threads) → Pinterest. Social inbox + moderation parked until Meta connects (live board row). Chris only creates each account and pastes one token per row. Plan: docs/SOCIAL-AUTOPILOT.md.
- [ ] [Claude] Homepage demo frame: static 2x image of the Inventory with one card per stage, EXAMPLE ribbon on the art, one-line caption + Try 5 scans free. Mockup approved 09-08; build after lockdown.
...
- [x] 09-10 Inventory value graph: a strip under the In play / Earned panel — the pile's asking value day by day from our own price series (a card counts from its scan day, a sold copy leaves on its sale day; 30d / 90d / 1y), GET /api/cards/value-history, test:inventoryvalue pins the math. Original thought: is it possible to give the inventory a graph of their listing value changing over time
- [x] 09-10 Condition now moves the saved price: changing Near Mint → Lightly Played (single card or Apply to all) re-quotes the ledger row and unlocks it, so Inventory and the live refresh follow the condition. Original thought: when a user changes the quality of the card such as near mint, the price seems to always stay the same
