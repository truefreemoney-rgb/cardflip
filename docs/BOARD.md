...
- [ ] [Claude] MAGIC LOCKDOWN (Chris 09-10: "nail this down like Pokémon"; his cards are 2025+): research → docs/MTG-IDENTIFICATION.md. PHASE 1 SHIPPED 09-10: the Magic scan has its own read (MTG_READ_SCHEMA: finish, treatment, marks, artist, copyright year, border, serial), the mirror stores artist/frame/border/frame_effects/promo_types/full_art/textless, the ranker breaks ties on them (cuePenalty; The List icon + original code/number lands on PLST; as-is reprint sets skip the year cue), the scan's finish is the price line, Magic photos upload at 1568px (List icon invisible at 1024). Catalog-image replay: 5/5 exact printing. NEXT: push cue columns to prod (scripts/push-mtg-cues.mjs), Chris scans 10 of his cards (foils + showcase/borderless) and reports misses; then phase 2 panel. Phase 2 = replay-scans mtg panel (≥95% clean) then Chris's 30-card phone batch (≥90%). Phase 3 = pick-by-picture on ties. Magic stays admins-only until the numbers pass.
