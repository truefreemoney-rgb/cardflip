...
- [x] 09-10 RUNNER UPGRADE 4 (Chris yes): GET /api/runner/status — read-only, its own secret (RUNNER_TOKEN, Bearer, constant-time compare): deploy sha, daily job, last-24h errors grouped, scan spend, last prod smoke run. RUNNER.md + the routine prompt tell it to look there first. Env var on Vercel + the routine: pending the classifier approval.
