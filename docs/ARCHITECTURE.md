...
| Auth | Email/password, DB sessions + cookie, optional TOTP with hashed backup codes. Admin console uses a separate panel cookie (`ADMIN_PANEL_USER/PASSWORD`), not a user session; an optional second login (`ADMIN_HELPER_USER/PASSWORD`, role `helper`) sees only the Tasks page and her own category — add note, photo, Run, Reply; no Merge, no other pages (09-10). | `src/lib/server/auth.ts`, `sessions.ts`, `totp.ts`, `adminGate.ts` |
