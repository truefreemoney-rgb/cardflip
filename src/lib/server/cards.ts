...
export function getCardForUser(id: string, userId: string): CardRecord | null {
  const row = db
...
export function setCardEbayDraft(
  id: string,
  userId: string,
...
export function setCardPhotoAt(id: string, userId: string, photoAt: number | null): void {
  db.prepare("UPDATE cards SET photo_at = ?, updated_at = ? WHERE id = ? AND user_id = ?").run(
    photoAt,
    Date.now(),
    id,
    userId,
...
export function cardPhotoAt(id: string): number | null {
  const row = db.prepare("SELECT photo_at FROM cards WHERE id = ?").get(id) as
    | { photo_at: number | null }
    | undefined;
 ...
 */
export function setCardEbayListing(
  id: string,
  userId: string,
  state: EbayListingState,
): CardRecord | null {
  const existing = getCardForUser(id, userId);
