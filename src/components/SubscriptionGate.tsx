...
import { isSubscribed } from "@/lib/client/auth";
import Paywall from "@/components/Paywall";

/**
 * Paid-only (09-04): every /app page needs an active subscription. Admins
...
export default function SubscriptionGate({ children }: { children: React.ReactNode }) {
  ...
  if (user.role === "admin" || isSubscribed(user) || pathname.startsWith("/app/account")) return <>{children}</>;
  return <Paywall />;
}
